...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-black/[0.04] bg-surface/85 backdrop-blur-md">
      <nav
        aria-label="Primary"
        className="mx-auto flex max-w-[1440px] items-center justify-between px-4 py-3 sm:px-6 sm:py-[18px] lg:px-12 xl:px-[110px] xl:py-[22px]"
      >
        {/* Brand */}
        <Link
          href="/"
        ...
        >
          <Logo wordmarkClassName="text-ink-strong" />
        </Link>
          ...
          <ul className="flex items-center gap-8 rounded-[60px] px-6 py-3 xl:gap-16 xl:px-8">
            {NAV_LINKS.map((link) => (
              <li key={link.label}>
                <Link
                  href={link.href}
                  className={`flex items-center gap-2 rounded-md px-1 py-0.5 text-[15px] font-medium leading-[18px] text-ink transition-colors hover:text-brand ${focusRing}`}
                >
                  {link.label}
        ...
        <button
          ...
          className={`relative z-50 flex size-10 items-center justify-center rounded-lg text-ink transition-colors hover:bg-black/[0.04] lg:hidden ${focusRing}`}
        >
      ...
      <div
        id="mobile-nav"
        ref={panelRef}
        className={`fixed inset-x-0 top-[60px] z-40 origin-top overflow-hidden border-b border-black/[0.06] bg-surface px-4 shadow-lg transition-all duration-300 ease-out lg:hidden ${
          open
